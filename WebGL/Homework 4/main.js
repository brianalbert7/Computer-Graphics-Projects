...
loader1.load('https://i.imgur.com/dea3ESa.png' , function(texture)
{
...
const groundMaterial = new THREE.MeshPhongMaterial({
    map: loader1.load('https://i.imgur.com/aNVqW2t.jpg'),
});
...
//Add cannons
//*********************
//each cannon will have a base (a box) that can rotate horizontally, and a body (a truncated cone that has a sphere in one end) that can tilt up or down. The tube will be children of the base in the scene graph, so that rotation of the box applies automatically to the tube
...
let cannonMaterial = new THREE.MeshBasicMaterial({
    map: loader1.load('https://i.imgur.com/HaGmXYa.jpg'),
});
...
let cannonBaseMaterial = new THREE.MeshBasicMaterial({
    map: loader1.load('https://i.imgur.com/e69Z1hI.jpeg'),
});
...
function rotateCannonsHorizontally(angle){//keeping them symmetrical. An angle of zero makes both cannons face the front, so we would add 90 degrees
	cannon1.quaternion.setFromAxisAngle(up, ((angle+90)*Math.PI/180));
	cannon2.quaternion.setFromAxisAngle(up, ((angle+90)*Math.PI/-180));
	//or
	//cannon1.rotation.z=...
}
	...
	for(let i=0;i<sphereList.length;i++){
		...
		for(let j=0;j<i;j++){
			...
			if(sphere2.position.distanceTo(sphere.position)<=cannonballRadius*2){
				...
				//2) more complex case: if they are not necessarily symmetrical, we need to calculate the contact normal vector (from one ball's center to the other's center), and flip the velocity components in this normal direction, assuming balls always have the same mass. See https://en.wikipedia.org/wiki/Elastic_collision 
				...
				//sphere.velocity.addScaledVector(projected, -1);
